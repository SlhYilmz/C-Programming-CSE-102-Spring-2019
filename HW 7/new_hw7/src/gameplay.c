                          ...
                          struct player* other)
{
    ...
    for (j = 0; j < (int32_t)BOARD_SIZE; j++)
    {
        if ((board[j].type == property) &&
            (board[j].owner.type == current->type))
        {
            board[j].owner = *current;
        }
    }
                        ...
                        int steps)
{
    ...
    old_pos = p->current_block_id;
    p->current_block_id = (p->current_block_id + steps) % (int)BOARD_SIZE;

    if (p->current_block_id < old_pos)
    {
                   ...
                   struct player* other)
{
    ...
    if (solvent == BANKRUPT)
    {
        for (j = 0; j < (int32_t)BOARD_SIZE; j++)
        {
            if (board[j].owner.type == current->type)
            {
                board[j].owner.type = noone;
                board[j].house_count = 0;
            }
        }
    }
