...
void gridPrinter(char *dict[], int coord[DICT_SIZE][4]){
	...
	printf("\nEnter the word: ");
	scanf("%s", &word);
	printf("Enter the coordinates of the word: ");
	scanf("%d %d", &coor1, &coor2);
}
...
void randomCharGrid(){
    ...
    for(i=0 ; i < DICT_SIZE ; i++){
        for(j=0 ; j < DICT_SIZE ; j++){
			grid[i][j] = randomCharacterCreator();
            printf("%c ", grid[i][j]);
        }
    	printf("\n");
    }
