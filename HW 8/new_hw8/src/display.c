...
static void cell_name(char* buf,
...
static void cell_price(char* buf,
                       const struct block* b)
{
...
static void cell_players(char* buf,
                         const struct block* b,
                         const struct player* p1,
...
{
    int32_t i;
    for (i = 0; i < (int32_t)(CELL_W + 3); i++)
    {
        printf("-");
    }
    for (i = 0; i < (int32_t)SIDE_MID_W; i++)
    {
        printf(" ");
    }
    for (i = 0; i < (int32_t)(CELL_W + 3); i++)
 ...
 */
static void print_side_row(const struct block* left,
                           ...
                           const struct player* p2)
{
    ...
    cell_name(name_l, left);
    cell_name(name_r, right);
    cell_price(price_l, left);
    cell_price(price_r, right);
...
static void print_deed(const struct block* b)
{
    ...
    /* outer top border */
    for (i = 0; i < (int32_t)(DEED_INNER + 2); i++)
    {
        printf("-");
    }
    printf("\n");
    ...
    /* title underline */
    printf("|");
    for (i = 0; i < (int32_t)DEED_INNER; i++)
    {
        printf("-");
    }
    printf("|\n");
...
/* -----------------------------------------------------------------------
 * Internal helper - find a block in the linked list by its id.
 * Returns NULL when not found.
 * --------------------------------------------------------------------- */
static const struct block* find_by_id(const struct block* head,
                                      int id)
{
    const struct block* b = head;
    while (b != NULL)
    {
        if (b->block_id == id)
        {
 ...
 * Part 2 - show_board
 *
 * Board layout (block indices, 24 blocks):
 *   Top row  (L->R): 0  1  2  3  4  5
 *   Right col(T->B): 6  7  8  9  10 11
 *   Bot row  (R->L): 12 13 14 15 16 17  (printed right-to-left: 17..12)
 *   Left col (B->T): 18 19 20 21 22 23  (printed bottom-to-top: 23..18)
 * --------------------------------------------------------------------- */
void show_board(struct block* board,
                ...
                struct player player_two)
{
    ...
    /* --- top row: blocks 0-5 --- */
    for (i = 0; i < (int32_t)BOARD_COLS; i++)
    {
        row[i] = find_by_id(board, i);
    }
    print_separator();
    print_row(row, p1, p2);

    /* --- side rows: left col 23->18, right col 6->11, inner merged --- */
    print_separator();
    print_side_row(find_by_id(board, 23), find_by_id(board, 6), p1, p2);

    print_side_separator();
    print_side_row(find_by_id(board, 22), find_by_id(board, 7), p1, p2);

    print_side_separator();
    print_side_row(find_by_id(board, 21), find_by_id(board, 8), p1, p2);

    print_side_separator();
    print_side_row(find_by_id(board, 20), find_by_id(board, 9), p1, p2);

    print_side_separator();
    print_side_row(find_by_id(board, 19), find_by_id(board, 10), p1, p2);

    print_side_separator();
    print_side_row(find_by_id(board, 18), find_by_id(board, 11), p1, p2);

    /* --- bottom row: blocks 17->12 (right to left) --- */
    print_separator();
    for (i = 0; i < (int32_t)BOARD_COLS; i++)
    {
        row[i] = find_by_id(board, 17 - i);
    }
    print_row(row, p1, p2);
    print_separator();
}
...
void show_properties(struct block* board)
{
    ...
    do
    {
        ...
        else
        {
            ...
            for (b = board; b != NULL; b = b->next)
            {
                if ((b->block_id == selection) && (b->type == property))
                {
                    valid = true;
                    print_deed(b);
                }
